...
static int dumpLLVMIR(mlir::ModuleOp module) {
  ...
  // NB: the module here was already processed and lowered to llvm-dialect
  ...
  // Configure the LLVM Module (this stuff is only needed for
...
// this is similar to dumpLLVMIR(), the only difference is that instead of
// lowering to LLVM IR we directly execute the LLVM dialect with a jit.
...
int main(int argc, char **argv) {
  ...
  // If we aren't exporting to non-mlir, then we are done (caso DumpMLIR).
